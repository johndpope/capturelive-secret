...
extension UIView {
    ...
    func decoratedButton()->UIButton {
        let button                          = UIButton(type:UIButtonType.Custom) as UIButton
        self.decorateButton(button )
        return button
    }
    
    func decoratedLabel()->UILabel {
        let lb                              = UILabel()
    ...
    func decoratedTextField()->UITextField {
        let tf                              = UITextField()
        self.decorateTextField(tf)
        return tf
    }
    
    func decorateTextField(field:UITextField) {
    ...
    func paddTextField(field:UITextField) {
    ...
    func decorateLabel(field:UILabel) {
        field.font                          = UIFont(name:CMFont.Regular.rawValue, size: CMFontSize.Large.rawValue);
        field.backgroundColor               = UIColor.whiteColor()
...
//        field.layer.borderWidth             = 1;
    ...
    func decorateButton(button:UIButton) {
        button.titleLabel?.font             = UIFont(name:CMFont.Regular.rawValue, size: CMFontSize.Large.rawValue)
        button.layer.masksToBounds          = true
        button.layer.cornerRadius           = 3
        button.setTitleColor(UIColor.whiteColor(), forState: UIControlState.Normal)
        button.setTitleColor(UIColor.whiteColor(), forState: UIControlState.Highlighted)
        button.setTitleColor(UIColor.grayColorAC(), forState: UIControlState.Selected)
    }
    
    func  heightPlusY(view: UIView)->CGFloat {
        return view.frame.size.height + view.frame.origin.y;
    }
    
    func widthPlusX(view: UIView)->CGFloat {
